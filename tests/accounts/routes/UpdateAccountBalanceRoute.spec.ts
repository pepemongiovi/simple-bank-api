...
import { clearDb, createAccountForTests, createBankForTests, createUserAndAuthenticateForTests } from "@shared/helpers/helper";
...
describe('GeAccountByIdRoute', () => {
  ...
  it('should update the balance from the account.', async () => {
    ...
    const res = await updateAccountBalanceForTests(
      request, appServer, account.id, newBalance, authToken
    )
  ...
  it('should update the balance from the account without authorization token.', async () => {
    ...
    const res = await updateAccountBalanceForTests(
      request, appServer, account.id, newBalance
    )
  ...
  it('should not be able to update a account with invalid account id.', async () => {
    ...
    const res = await updateAccountBalanceForTests(
      request, appServer, fakeId, newBalance, authToken
    )
...
export const updateAccountBalanceForTests = async (request: any, appServer: any, account_id: string, balance: number, authToken = "") => {
