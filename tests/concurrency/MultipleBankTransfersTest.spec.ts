...
import { clearDb, createAccountForTests, createBankForTests, createUserAndAuthenticateForTests, getAccountByIdForTests, isTransactionEquals, makeDepositForTests, makeTransferForTests, makeWithdrawForTests } from "@shared/helpers/helper";
...
describe('MultipleBankTransfersTest', () => {
  ...
  beforeEach(async () => {
    ...
    const depositOtherAccountRes = await makeDepositForTests(
      request, appServer, valueToBeDeposited, otherAccount.id, authToken
    )
  ...
  it('should handle concurrency on multiple transfer', async () => {
    ...
    const accountsDeposits = [
      Promise.resolve(makeTransferForTests(
        request, appServer, valueToBeTransfered, account.id, otherAccount.id, authToken
      )),
      Promise.resolve(makeTransferForTests(
        request, appServer, valueToBeTransfered, account.id, otherAccount.id, authToken
      )),
    ...
    const otherAccountsDeposit = [
      Promise.resolve(makeTransferForTests(
        request, appServer, valueToBeTransfered, otherAccount.id, account.id, authToken
      )),
      Promise.resolve(makeTransferForTests(
        request, appServer, valueToBeTransfered, otherAccount.id, account.id, authToken
      )),
    ...
    await Promise.all(
      [ ...accountsDeposits, ...otherAccountsDeposit ]
    ).then((responses) => {
    ...
    const otherAccountAfterTransferRes = await getAccountByIdForTests(
      request, appServer, otherAccount.id, authToken
    )
    ...
    expect(otherAccountAfterTransferRes.body.balance).toBe(otherAccountInitialBalance)
  })
})
