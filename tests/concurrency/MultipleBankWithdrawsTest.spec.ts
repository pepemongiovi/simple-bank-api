...
import { clearDb, createAccountForTests, createBankForTests, createUserAndAuthenticateForTests, getAccountByIdForTests, isTransactionEquals, makeDepositForTests, makeWithdrawForTests } from "@shared/helpers/helper";
...
describe('MultipleBankWithdrawsTest', () => {
  ...
  it('should handle concurrency on multiple withdraws', async () => {
    ...
    await Promise.all([
      Promise.resolve(makeWithdrawForTests(
        request, appServer, valueToBeWithdrawn, account.id, authToken
      )),
      Promise.resolve(makeWithdrawForTests(
        request, appServer, valueToBeWithdrawn, account.id, authToken
      )),
    ...
    const transactionsCost = (valueToBeWithdrawn * (valuePercentageForBonus/100)) * numberOfDeposits
