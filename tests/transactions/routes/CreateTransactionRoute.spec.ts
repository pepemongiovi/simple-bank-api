...
describe('CreateTransactionRoute', () => {
  ...
  it('should create a new transaction', async () => {
    ...
    const transactionRes = await createTransactionForTests(
      request, appServer, account.id, account.id, valueToBeDeposited,
      TransactionType.DEPOSIT, bonusValue, transactionCost, authToken
    )
  ...
  it('should not create a transaction withou a authorization token.', async () => {
    ...
    const transactionRes = await createTransactionForTests(
      request, appServer, account.id, account.id, valueToBeDeposited,
      TransactionType.DEPOSIT, bonusValue, transactionCost
    )
  ...
  it('should not create a transaction with a invalid account id.', async () => {
    ...
    const transactionFakeFromAccountIdRes = await createTransactionForTests(
      request, appServer, fakeAccountId, account.id, valueToBeDeposited,
      TransactionType.DEPOSIT, bonusValue, transactionCost, authToken
    )

    const transactionFakeToAccountIdRes = await createTransactionForTests(
      request, appServer, account.id, fakeAccountId, valueToBeDeposited,
      TransactionType.DEPOSIT, bonusValue, transactionCost, authToken
    )
  ...
  it('should not be able to create a transaction with a value that is not greater than zero.', async () => {
    ...
    const negativeValueTranscRes = await createTransactionForTests(
      request, appServer, account.id, account.id, negativeValueToBeDeposited,
      TransactionType.DEPOSIT, bonusValue, transactionCost, authToken
    )

    expect(valueZeroTranscRes.statusCode).toEqual(400)
    expect(valueZeroTranscRes.body.message).toEqual('Transaction value must be greater than zero.')

    expect(negativeValueTranscRes.statusCode).toEqual(400)
    expect(negativeValueTranscRes.body.message).toEqual('Transaction value must be greater than zero.')
  });
})

export const createTransactionForTests = async (request: any, appServer: any,
  from_account_id: string, to_account_id: string, value: number, type: string,
  bonusValue: number, transactionCost: number, authToken = ""
) => {
