...
describe('GetUserByIdRoute', () => {
  ...
  it('should get a existing user', async () => {
    const getByIdRes = await getUserByIdForTests(
      request, appServer, newUser.id, authToken
    )
  ...
  it('should not be able to get a user with an invalid id.', async () => {
    ...
    const getByIdRes = await getUserByIdForTests(
      request, appServer, fakeUserId, authToken
    )
...
export const getUserByIdForTests = async (request: any, appServer: any, user_id: string, authToken = "") => {
