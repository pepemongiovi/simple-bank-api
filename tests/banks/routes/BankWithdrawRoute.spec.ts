...
describe('BankWithdrawRoute', () => {
  ...
  it('should make a new withdraw', async () => {
    ...
    const withdrawRes = await makeWithdrawForTests(
      request, appServer, valueToBeWithdrawed, account.id, authToken
    )

    const bonusValue = 0
    const withdrawPercentageCost = 1
    const transactionCost = valueToBeWithdrawed * (withdrawPercentageCost/100)
    const valueWithTransactionCost = valueToBeWithdrawed + transactionCost

    const finalAccountBalance = depositRes.body.updatedAccount.balance - valueWithTransactionCost

    const expectedTransactionIsValid = isTransactionEquals(
      withdrawRes.body.transaction, account.id, account.id, TransactionType.WITHDRAW,
      valueToBeWithdrawed, bonusValue, transactionCost
    )
    ...
    expect(withdrawRes.body.updatedAccount.balance).toBe(
      finalAccountBalance
    );
  })
  ...
  it('should not withdraw without a authorization token.', async () => {
    const res = await makeDepositForTests(
      request, appServer, 100, account.id
    )
  ...
  it('should not be able to withdraw to an account with invalid account_id.', async () => {
    ...
    const res = await makeDepositForTests(
      request, appServer, valueToBeWithdrawed, fakeAccountId, authToken
    )

    expect(res.statusCode).toEqual(404)
    expect(res.body.message).toEqual('No account found with the given account_id.')
  });
  ...
  it('should not be able to withdraw a value that is not greater than zero to an account.', async () => {
    ...
    const negativeWithdraw = await makeWithdrawForTests(
      request, appServer, negativeValueToBeDeposited, account.id, authToken
    )

    expect(valueZeroWithdraw.statusCode).toEqual(400)
    expect(valueZeroWithdraw.body.message).toEqual('Withdraw value must be greater than zero.')
