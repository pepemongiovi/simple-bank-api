...
describe('BankTransferRoute', () => {
  beforeEach(async () => {
    ...
    const otherUser = (await createUserAndAuthenticateForTests(
      request, appServer, 'Jader Freitas', '034.037.960-00'
    )).user
    ...
    const otherNewAccountResponse = await createAccountForTests(
      request, appServer, otherUser.id, bank.id, authToken
    );
  ...
  it('should make a new transfer', async () => {
    ...
    const transferRes = await makeTransferForTests(
      request, appServer, valueToBeTransfered, account.id,
      otherAccount.id, authToken
    )

    const bonusValue = 0
    const transactionCost = 0

    const finalAccountBalance = depositRes.body.updatedAccount.balance - valueToBeTransfered

    const expectedTransactionIsValid = isTransactionEquals(
      transferRes.body.transaction, account.id, otherAccount.id,
      TransactionType.TRANSFER, valueToBeTransfered, bonusValue, transactionCost
    )
    ...
    expect(transferRes.body.updatedAccount.balance).toBe(
      finalAccountBalance
    );
  })
  ...
  it('should not be able to transfer with an invalid account id.', async () => {
    ...
    const invalidFromAccountIdTransferRes = await makeTransferForTests(
      request, appServer, valueToBeTransfered, fakeAccountId, account.id, authToken
    )
    const invalidToAccountIdTransferRes = await makeTransferForTests(
      request, appServer, valueToBeTransfered, account.id, fakeAccountId, authToken
    )
  ...
  it('should not be able to transfer a value that is not greater than zero to an account.', async () => {
    ...
    const valueZeroTransferRes = await makeTransferForTests(
      request, appServer, valueToBeTransfer, account.id,
      otherAccount.id, authToken
    )

    const negativeTransferRes = await makeTransferForTests(
      request, appServer, negativeValueToBeDeposited, account.id,
      otherAccount.id, authToken
    )
  ...
  it('should not be able to transfer a value greater than the balance of who is transfering.', async () => {
    ...
    expect(valueZeroTransferRes.statusCode).toEqual(400)
    expect(valueZeroTransferRes.body.message).toEqual(
      `Insufficient balance. The account's current balance is R$ ${account.balance.toFixed(2)}.`
    )
  });
})
