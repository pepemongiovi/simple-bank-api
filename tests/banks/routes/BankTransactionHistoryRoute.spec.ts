...
describe('BankTransactionHistoryRoute', () => {
  ...
  it('should get transaction history from account.', async () => {
    ...
    const transactionHistoryRes = await getTransactionHistoryForTests(
      request, appServer, account.id, startOfCurrentDay, oneMonthFromNow, authToken
    )
  ...
  it('should not deposit without a authorization token.', async () => {
    ...
    const transactionHistoryRes = await getTransactionHistoryForTests(
      request, appServer, account.id, startOfCurrentDay, endOfCurrentDay
    )
  ...
  it('should not be able to deposit to an account with invalid account_id.', async () => {
    ...
    const transactionHistoryRes = await getTransactionHistoryForTests(
      request, appServer, fakeAccountId, startOfCurrentDay, endOfCurrentDay, authToken
    )
  ...
  it('should not be able to create a transfer with invalid date interval.', async () => {
    ...
    const transactionHistoryRes = await getTransactionHistoryForTests(
      request, appServer, account.id, endOfCurrentDay, startOfCurrentDay, authToken
    )
  ...
  it('should not be able to create a transfer with invalid start date.', async () => {
    ...
    const transactionHistoryRes = await getTransactionHistoryForTests(
      request, appServer, account.id, startOfCurrentDay, endOfCurrentDay, authToken
    )
...
export const getTransactionHistoryForTests = async (request: any, appServer: any, account_id: string, from_date: Date, to_date: Date, authToken = "") => {
