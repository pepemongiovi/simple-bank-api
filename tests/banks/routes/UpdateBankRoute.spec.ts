...
import { clearDb, createBankForTests, createUserAndAuthenticateForTests } from "@shared/helpers/helper";
...
describe('UpdateBankByIdRoute', () => {
  ...
  it('should update a existing bank', async () => {
    ...
    const updateBankRes = await updateBankForTests(
      request, appServer, modifiedBank, authToken
    )
  ...
  it('should not update bank without authorization token.', async () => {
    ...
    const updateBankRes = await updateBankForTests(
      request, appServer, modifiedBank
    )
  ...
  it('should not be able to update with an invalid id.', async () => {
    ...
    const updateBankRes = await updateBankForTests(
      request, appServer, fakeBank, authToken
    )

    expect(updateBankRes.statusCode).toEqual(404)
    expect(updateBankRes.body.message).toEqual('No bank found with the given id.')
  });
  ...
  it('should not be able to update a bank with a taken cnpj.', async () => {
    ...
    const createBankItauRes = await createBankForTests(
      request, appServer, 'Itaú', '20.543.290/0001-27', authToken
    )
    ...
    const updateBankRes = await updateBankForTests(
      request, appServer, modifiedBankBB, authToken
    )

    expect(updateBankRes.statusCode).toEqual(403)
    expect(updateBankRes.body.message).toEqual('Bank with this CNPJ already exists.')
  });
...
export const updateBankForTests = async (request: any, appServer: any, bank: Bank, authToken = "") => {
